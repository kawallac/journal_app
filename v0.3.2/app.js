document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateNavAndActionsUI() {
    ...
    // Delete: only when we have a current entry
    if (deleteBtnTop) {
      deleteBtnTop.disabled = !hasEntries;
    }

    // Prev/Next: disable at edges
    if (prevBtn && nextBtn) {
      if (!hasEntries) {
  ...
  // ----- Dragging a NEW tag from "New Tag" button -----
  ...
  function deleteCurrentEntry() {
    ...
    // If we deleted an entry, go to previous if possible, otherwise to first
    const newIdx = Math.min(entries.length - 1, Math.max(0, getCurrentEntryIndex()));
  ...
  function init() {
    ...
    if (entries.length === 0) {
      ...
      // By default, go to the most recently updated entry
