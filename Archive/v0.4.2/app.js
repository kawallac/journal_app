/* ============================================
   ...
   Description:
     ...
     - Prev/Next navigation (now chronological)
     - Export JSON button
   ============================================ */
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function migrateEntry(entry) {
    ...
    // 3) Timestamps: ensure createdAt / updatedAt exist
  ...
  function getChronologicallySortedEntries() {
    // Returns a NEW array sorted by journal date (ascending),
    // then by createdAt timestamp (ascending) for entries on the same date.
  ...
  function deleteCurrentEntry() {
    ...
    // Determine current entry's position in chronological order before deletion
    ...
    // After deletion, pick the closest chronological neighbor
    ...
    if (newIdx >= sortedAfter.length) {
      newIdx = sortedAfter.length - 1;
    }
  ...
  function openEntryFromSearch(entryId) {
    ...
    if (idx === -1) return;
    goToEntryAtIndex(idx); // idx interpreted chronologically via helper
  }
